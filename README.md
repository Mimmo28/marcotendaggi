# Mega Shop Arredi - Documentazione Tecnica

Benvenuto nel codice sorgente del nuovo sito vetrina di "Mega Shop Arredi", interamente progettato per essere ultra-veloce, sicuro e profondamente ottimizzato per i motori di ricerca.
...
2. **Tailwind CSS**: Il framework utilizzato per il design e lo styling. Ci ha permesso di ottenere un'estetica premium: palette colori personalizzata (con l'iconico oro/giallo `#E4A11B`), animazioni soffici come lo zoom delle foto e una totale responsività per cellulari, tablet e pc.

## 🛠️ Cosa abbiamo costruito (Feature List)

- **Layout e Componenti Globali**: Un *Header* (con logo a scomparsa e menu mobile), un *Footer* ricolmo di info legali e contatti societari, un pulsante galleggiante laterale (FAB) per la chat rapida tramite WhatsApp, e un complesso sistema nativo di `Lightbox` per cliccare e ingrandire le immagini dei tuoi mobili.
- **Pagine Principali**: 
  - *Home (`/`)*: Un ingresso esplosivo con slider in apertura e griglie per la panoramica dei marchi e delle categorie.
  - *Collezioni (`/collezioni/[nome]`)*: Varie gallerie fotografiche automatiche e massive (divani, soggiorni, cucine, camere, pareti) collegate intelligemente per leggere le cartelle fisiche `public/images/`.
  - *Chi Siamo (`/chi-siamo`)*: Lo storytelling solido dell'azienda, il focus d'impatto sul tuo "store" fisico.
  - *Contatti (`/contatti`)*: Mappe interattive di Google coordinate con i colori tenui.
...
- **Security Headers**: Hanno implementato direttive forti in un file `public/_headers` (anti-Clickjacking e traffico forzato HSTS) e un tag ferreo di *Content-Security-Policy (CSP)* nel Layout.
...
- **LLMs.txt**: Coniando un nuovissimo standard tecnologico, il file `public/llms.txt` fa da ponte pulito perché chat intelligenti come ChatGPT e Gemini trovino e raccomandino "Mega Shop Arredi".

## 👨‍💻 Comandi Utili per il Lato Tecnico
Apri il terminale dentro la cartella `megashoparredi` e sperimenta:
